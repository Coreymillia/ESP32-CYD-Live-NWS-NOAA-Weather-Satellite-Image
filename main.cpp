...
// Setup: On every boot an AP (WeatherCore_Setup) is created for configuration.
...
void setup() {
  ...
  // Run captive portal — loads saved settings, shows AP instructions on screen,
  // user enters WiFi + camera (or clicks No Changes), then portal closes.
  wcInitPortal();
  while (!portalDone) {
    wcRunPortal();
    delay(5);
